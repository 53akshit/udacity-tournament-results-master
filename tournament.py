...
def playerStandings():
    ...
    db = connect()
    c = db.cursor()
    c.execute("SELECT won_match.id,won_match.name,won_match.win,total_match.matches from won_match,total_match where won_match.id =total_match.id order by win desc")
    result = []
    for a in c.fetchall():
        result.append(a)
    db.commit()
...
def swissPairings():
    ...
    db = connect()
    c = db.cursor()
    c.execute("SELECT * FROM won_match;")
    rows = c.fetchall()
    db.close()
    i = 0
    result = []
